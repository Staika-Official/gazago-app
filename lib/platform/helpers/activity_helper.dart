...
double calculateAvgSpeed(List<double> speedList) {
  if (speedList.isNotEmpty) {
    double sumSpeed =
        speedList.fold(0, (summedValue, speed) => summedValue + speed);
...
double convertMStoKMH(double avgSpeedMS) {
  double result = (avgSpeedMS * 3600 / 1000);
  return result;
}
...
double calculateTotalDistance(List<double> distanceList) {
  return distanceList.fold(
      0, (summedValue, distance) => summedValue + distance);
}
...
bool batchIsInProgress() {
  ...
  DateTime batchProcessStartDateTime = DateTime.utc(
      currentDateTime.year, currentDateTime.month, currentDateTime.day, 14, 55);
  DateTime batchProcessEndDateTime = DateTime.utc(
      currentDateTime.year, currentDateTime.month, currentDateTime.day, 15, 5);
  if (currentDateTime.isAfter(batchProcessStartDateTime) &&
      currentDateTime.isBefore(batchProcessEndDateTime)) {
    return true;
  } else {
    return false;
  }
