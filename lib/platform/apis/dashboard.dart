...
class DashboardApi {
  static Future<Response> getDailyRankingList(date, page, size) async {
    return await Api.client(
      serviceUrl: '${ServiceUrl.dashboardV2Service}/ranking/$date',
  ...
  static Future<Response> getDailyRankingMyRank(date, userId) async {
    return await Api.client(
      serviceUrl: '${ServiceUrl.dashboardService}/ranking/$date/users/$userId',
      showLoading: false,
    ).get('');
  }
  ...
  static getUserRewardStatistics(String userId, String dateFormat) async {
    return await Api.client(
      serviceUrl: '${ServiceUrl.dashboardV2Service}/users/$userId/statistics/month/$dateFormat',
      showLoading: false,
    ).get('');
  }
  ...
  static getTodayRewardTik(date) async {
    return await Api.client(
      serviceUrl: '${ServiceUrl.dashboardService}/total-reward-tik/$date',
      showLoading: false,
    ).get('');
  }
}
