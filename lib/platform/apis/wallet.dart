...
class WalletApi {
  static Future<Response> generateSpendingWallet() async {
    return await Api.client(serviceUrl: ServiceUrl.spendingWalletService).post('/generate-wallet');
  }

  static Future<Response> getSpendingWalletBalance() async {
    return await Api.client(serviceUrl: ServiceUrl.spendingWalletService).get('/wallet/balance', queryParameters: {'client-id': 'GAZAGO'});
  }

  static Future<Response> getSpendingWalletTransactions(String publicKey, int size) async {
    return await Api.client(serviceUrl: ServiceUrl.spendingWalletService).get('//api/spending/wallet/$publicKey/transactions', queryParameters: {'client-id': 'GAZAGO', 'size': size});
  }
